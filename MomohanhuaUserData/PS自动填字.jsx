...
var pBubbleMeta = new RegExp("(\\d{1,5},\\d{1,5},\\d{1,5},\\d{1,5})~(\\d{1,3})~(Horizontal|Vertical)~(Left Title|Left|Center|Right)~(\\d{1,3},\\d{1,3},\\d{1,3})~(.*)", "g");
...
var pHex = new RegExp("([a-fA-f0-9]{2})([a-fA-f0-9]{2})([a-fA-f0-9]{2})", "g");
 ...
 */
function setFormatting(start, end, fontPostScriptName, fontSize, hexString, styleNum) {
    ...
    if (activeLayer.kind === LayerKind.TEXT) {
        ...
        if ((activeLayer.textItem.contents !== "") && (start >= 0) && (end <= activeLayer.textItem.contents.length)) {
            ...
            if (pHex.test(hexString)) {
                ...
                alert(red.toString() + ',' + green.toString() + ',' + blue.toString())
            ...
            var underline = parseInt(styleNum / 4)
            var bold = parseInt((styleNum - underline * 4) / 2)
            var italic = styleNum % 2
...
function setStyle(start, end, styleOption) {
    //Sanity checking: is the active layer a text layer?
    var activeLayer = activeDocument.activeLayer
    if (activeLayer.kind === LayerKind.TEXT) {
        //More checking: does the text layer have content, and are start and end set to reasonable values?
        if ((activeLayer.textItem.contents !== "") && (start >= 0) && (end <= activeLayer.textItem.contents.length)) {
            ...
            if (styleOption === '') {
                ...
                formatting.putInteger(idtextOverrideFeatureName, 808465976);
                formatting.putInteger(idtypeStyleOperationType, 3);
                if (styleOption === 'cross') {
                    //直排内横排
                    formatting.putEnumerated(idbaselineDirection, idbaselineDirection, idcross);
                } else {
                    //'stream'
                    //标准垂直罗马对齐方式
                    formatting.putEnumerated(idbaselineDirection, idbaselineDirection, idwithStream);
                }
            }
...
function autoTracking(tracking) {
    var reference = new ActionReference();
    reference.putProperty(idproperty, idtextStyle);
    reference.putEnumerated(idtextLayer, idordinal, idtargetEnum);
    ...
    var textAction = new ActionDescriptor();
    textAction.putInteger(idtextOverrideFeatureName, 808465465);
    textAction.putInteger(idtypeStyleOperationType, 3);
    textAction.putInteger(idtracking, tracking);

    action.putObject(idto, idtextStyle, textAction);
    executeAction(idset, action, DialogModes.NO);
}
...
function processFolderPic(mediaType, folderPath, bgFileList, pageContents) {
    ...
    for (var h = 0; h < bgFileList.length; h++) {
        ...
        if (whitenFile.exists) {
            var bubbleContents = pageContents[pin];
            pin += 1
            ...
            for (var b = 0; b < bubbleContents.length; b++) {
                ...
                var coors, centerX, centerY, textW, textH, colors, coorX, coorY
                ...
                if (pBubbleMeta.test(info_str)) {
                    ...
                    coors = RegExp.$1;
                    var coor_infos = coors.split(',');

                    centerX = parseInt(coor_infos[0]);
                    centerY = parseInt(coor_infos[1]);
                    textW = parseInt(coor_infos[2]);
                    textH = parseInt(coor_infos[3]);
                    ...
                    colors = RegExp.$5;
                    var color_infos = colors.split(',');

                    red = parseInt(color_infos[0]);
                    green = parseInt(color_infos[1]);
                    blue = parseInt(color_infos[2]);

                    font_name = RegExp.$6;

                    if (direction === 'Horizontal') {
                        coorY = centerY - 0.5 * textH + 0.9 * font_size
                        if (alignment === 'Left') {
                            coorX = centerX - 0.5 * textW
                        } else {
                            coorX = centerX
                        }
                    } else {
                        coorX = centerX + 0.5 * textW - 0.5 * font_size
                        coorY = centerY - 0.5 * textH
                    }
        ...
        if (currentFolder != null) {
            ...
            for (var p = 0; p < jpgList.length; p++) {
                ...
                if (!pLettered.test(currentFileStem)) {
                    backgroundFileList.push(currentJpg);
                }
            }

            if (backgroundFileList.length >= pageContents.length && backgroundFileList.length > 0) {
                processFolderPic(mediaType, folderPath, backgroundFileList, pageContents);
            } else {
